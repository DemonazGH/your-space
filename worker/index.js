...
export function validate(x) {
  const block=blocks.find(b=>b.id===x?.blockId);
  if(!block || x.schemaVersion!==1 || x.consent!==true || !['ru','en'].includes(x.language))throw Error('Invalid envelope');
...
export default {
 async fetch(request,env) {
  ...
  const path=`${mode}/${data.sessionId}/${data.blockId}.json`;
